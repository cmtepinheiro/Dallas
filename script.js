/* ==========================================================
   DALLAS AUTONOMOUS — FRONTEND CONTROLLER (V16 PRO)
   Enhanced with secure form handling, investor VDR modal,
   skip intro control, and motion/accessibility preferences.
   ========================================================== */
...
function runBootSequence(onComplete) {
    ...
    // Skip Intro Button Injection
    if (loader && !document.getElementById("skipIntroBtn")) {
        const skipBtn = document.createElement("button");
        skipBtn.id = "skipIntroBtn";
        skipBtn.className = "skip-intro-btn";
        skipBtn.textContent = "SKIP INTRO ⏩";
        skipBtn.setAttribute("aria-label", "Skip boot introduction");
    ...
    startVideo.addEventListener("playing", holdAtStart, { once: true });
    ...
    const revealDelay = BOOT_LINES.length * stepDelay + 250;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // INVESTOR VDR MODAL INJECTION
    const investmentSection = document.getElementById("investment");
    if (investmentSection && !document.getElementById("vdrModal")) {
        const vdrContainer = document.createElement("div");
        vdrContainer.className = "vdr-trigger-wrap";
        vdrContainer.innerHTML = `
            <button type="button" id="openVdrBtn" class="btn-primary vdr-btn" style="margin-top: 2rem; background: transparent; border: 1px solid var(--accent); color: var(--accent);">
                🔒 Request Investor Data Room (VDR) Access
            </button>
        `;
        investmentSection.appendChild(vdrContainer);

        const modal = document.createElement("div");
        modal.id = "vdrModal";
        modal.className = "vdr-modal";
        modal.innerHTML = `
            <div class="vdr-modal-content">
                <button type="button" id="closeVdrModal" class="vdr-close">&times;</button>
                <h3>Investor Secure Data Room</h3>
                <p>Institutional investors and funds may request access to the complete financial model, patent documentation, and FAPESP compliance dossier.</p>
                <form id="vdrForm" class="vdr-form">
                    <input type="text" name="investor_name" placeholder="Full Name / Managing Partner" required>
                    <input type="email" name="investor_email" placeholder="Institutional Email (@fund.com)" required>
                    <input type="text" name="fund_name" placeholder="Fund / Institution Name" required>
                    <select name="ticket_size" required>
                        <option value="" disabled selected>Select Expected Ticket Size</option>
                        <option value="seed">R$ 500k - R$ 2M (Seed)</option>
                        <option value="series-a">R$ 2M - R$ 10M (Series A)</option>
                        <option value="strategic">Strategic / CVC / M&A</option>
                    </select>
                    <label class="vdr-nda-label">
                        <input type="checkbox" required> I agree to execute a digital NDA prior to document download.
                    </label>
                    <button type="submit" class="btn-primary">Request Secure Credentials</button>
                    <p id="vdrStatus" class="contact-form-status" hidden style="margin-top: 1rem; color: var(--accent);"></p>
                </form>
            </div>
        `;
        document.body.appendChild(modal);

        const openBtn = document.getElementById("openVdrBtn");
        const closeBtn = document.getElementById("closeVdrModal");
        const vdrForm = document.getElementById("vdrForm");
        const vdrStatus = document.getElementById("vdrStatus");
        ...
        if (vdrForm) {
            vdrForm.addEventListener("submit", (e) => {
                e.preventDefault();
                vdrStatus.hidden = false;
                vdrStatus.textContent = "Credentials verification in progress. Our investor relations team will dispatch access to your verified email within 2 hours.";
                vdrForm.reset();
            });
        }
    }
