...
function runBootSequence(onComplete) {
    ...
    // Some mobile browsers throttle preload="auto" on cellular connections
    // unless the video is also autoplaying — and some (notably iOS Safari)
    // won't decode video at all while it's opacity:0 or display:none, so
    // it stays genuinely on-screen (just covered by an opaque backdrop)
    // the whole time. We let it autoplay muted right away purely to force
    // real buffering, then pause it almost instantly so it holds near
    // frame 0 — ready to resume cleanly once the boot log finishes.
    ...
    setTimeout(() => {
        ...
        // Always play muted — the safest, universally reliable path for
        // autoplay with no user gesture. Unmuting programmatically here
        // risks browsers (notably Chrome on Android) pausing the video
        // outright when audio isn't allowed to start, which is worse
        // than just keeping it silent.
