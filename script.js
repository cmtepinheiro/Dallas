...
function runBootSequence(onComplete) {
    ...
    if (reducedMotion) {
        // Respect reduced motion (no staggered animation, no beep) but
        // still show the systems log briefly instead of skipping entirely.
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Preload the hero landing video right away so it's ready to play the
    // instant the boot screen clears — otherwise it starts immediately on
    // page load and plays out (partly or fully) hidden behind the loader.
    if (heroVideo) {
...
// =====================================================
// RADAR SWEEP — driven entirely by JS (requestAnimationFrame)
// instead of CSS transform-box or SMIL, both of which have
// spotty support on mobile browsers (notably iOS Safari).
// Also drives the logo blip flash on the same clock so the
// two stay perfectly in sync.
// =====================================================
(function initRadarSweep() {
    const sweep = document.getElementById("mapSweep");
    const blip = document.querySelector(".map-blip");
    if (!sweep) return;

    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reducedMotion) {
        sweep.setAttribute("transform", "rotate(45 125 125)");
        if (blip) blip.style.opacity = "0.9";
        return;
    }

    const CYCLE_MS = 4000;
    const BLIP_ANGLE_FRACTION = 45 / 360; // matches the blip's position on the ring
    let rafId = null;
    ...
    function tick(now) {
        try {
            ...
            sweep.setAttribute("transform", `rotate(${angle.toFixed(2)} 125 125)`);

            if (blip) {
                const distance = Math.abs(progress - BLIP_ANGLE_FRACTION);
                const wrapped = Math.min(distance, 1 - distance);
                if (wrapped < 0.02) {
                    blip.style.opacity = "1";
                    blip.style.filter = "drop-shadow(0 0 5px var(--accent)) drop-shadow(0 0 9px var(--accent))";
                } else {
                    blip.style.opacity = "0.35";
                    blip.style.filter = "none";
                }
            }
        } catch (err) {
            // Never let a stray runtime error permanently freeze the sweep —
    ...
    // iOS Safari (and other mobile browsers) can silently drop the rAF loop
    // when the tab is backgrounded (app switch, screen lock, etc.) and fail
    // to resume it on return. Force a restart whenever the page becomes
    // visible again — harmless if the loop was already running fine.
    document.addEventListener("visibilitychange", () => {
        if (!document.hidden) {
