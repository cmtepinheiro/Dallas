...
function animateGauge(el) {
    ...
    const needleAngle = DIAL_START_ANGLE + (percent * DIAL_SWEEP) / 100;

    const needle = el.querySelector("[data-needle]");
    const numberEl = el.querySelector(".gauge-number");

    if (needle) {
        needle.style.transform = `rotate(${(needleAngle - DIAL_START_ANGLE).toFixed(2)}deg)`;
    }
    ...
    if (numberEl) {
        ...
        function tick(now) {
            const progress = Math.min((now - start) / duration, 1);
            const eased = 1 - Math.pow(1 - progress, 3);
            numberEl.textContent = Math.round(eased * value);
